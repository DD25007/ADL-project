...
class KGANet(nn.Module):
    ...
    Args:
        ...
        backbone: Optional pre-initialized backbone
    """
...
def train_joint_epoch(
    ...
    for _ in pbar:
        ...
        vid_scaler.unscale_(main_optimizer)
        torch.nn.utils.clip_grad_norm_(
            list(image_model.parameters()) + list(video_model.parameters()),
            max_norm=1.0,
        )

        vid_scaler.step(main_optimizer)
        vid_scaler.update()
...
def train_joint_kga_net(
    ...
    # Paper: "All images of the BUSI dataset are adopted to train our KGA-Net"
    # Use ALL images for training (no validation split for images)
    train_image_dataset = BUSIImageDataset(
        image_root_dir, image_annotation_file, train_img_transform
    )

    # For image validation monitoring (uses same images as training)
    ...
    logger.info(f"Image training samples: {len(train_image_dataset)} (ALL images used)")
    logger.info(
        f"Image validation samples: {len(val_image_dataset)} (same as train, for monitoring)"
    )
    ...
    # Paper's LR schedule: warmup + step decay at 4000 and 6000
    def lr_lambda(current_iter):
        if current_iter < 1000:  # Warmup for first 1000 iterations
            return float(current_iter) / 1000.0
        elif current_iter < 4000:  # Keep at 1.0 until 4000
            return 1.0
        elif current_iter < 6000:  # Reduce by 10x at 4000
            return 0.1
        else:  # Reduce by another 10x at 6000
            return 0.01
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            # Note: image_val_loader uses same images as training (paper's setup)
            # This is just for monitoring training progress, not true validation
        ...
        logger.info(
            f"Val   - Image Acc: {val_img_acc:.2f}% (train set), Video Acc: {val_vid_acc:.2f}%"
        )
        ...
        # Early stopping check (only based on video performance)
        if early_stopping:
