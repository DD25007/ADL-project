...
def evaluate_model_detailed(model, dataloader, device, model_name="Model"):
    ...
    with torch.no_grad():
        for frames, labels, keyframe_indices in tqdm(
...
def load_model_and_evaluate(
    checkpoint_path, dataloader, device, model_name, num_classes=2, feature_dim=2048
):
...
def run_complete_analysis(checkpoint_dir, val_loader, device, output_dir="plots"):
    ...
    plot_confusion_matrices(
        results_dict, save_path=f"{output_dir}/2_confusion_matrices.png"
    )
    ...
    plot_performance_comparison(
        results_dict, save_path=f"{output_dir}/4_performance_comparison.png"
    )

    # 5. Per-class performance
    plot_per_class_performance(
        results_dict, save_path=f"{output_dir}/5_per_class_performance.png"
    )
...
def analyze_models(results_dict):
    ...
    for name, results in results_dict.items():
        ...
        if precision_gap < 0.05 and recall_gap < 0.05:
            print(
                f"      • {name}: HIGHLY BALANCED (precision gap: {precision_gap*100:.1f}%, recall gap: {recall_gap*100:.1f}%)"
            )
...
if __name__ == "__main__":
    ...
    # Configuration
    CHECKPOINT_DIR = "notebooks/video_only_checkpoints"
    DATA_ROOT = "./data"
    VAL_ANNOTATION = "imagenet_vid_val.json"
    BATCH_SIZE = 4
    NUM_FRAMES = 32
    OUTPUT_DIR = "test_analysis_plots"

    # Import your dataset loader
    from kaustav_kga import video_dataloader
